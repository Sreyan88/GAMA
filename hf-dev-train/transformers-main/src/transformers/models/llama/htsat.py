    ...
    def forward(self, x, longer_idx = None):
        if (self.enable_fusion) and (self.fusion_type in ['daf_2d','aff_2d','iaff_2d']):
            ...
            if len(longer_idx) > 0:
                ...
                # print(global_x.dtype)
                # print(longer_idx.dtype)
                # print(local_x.dtype)
                global_x[longer_idx] = self.fusion_model(global_x[longer_idx],local_x).to(torch.bfloat16).to(torch.bfloat16)
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x, longer_idx = None):
        ...
        # print("fine_grained_latent_output:",fine_grained_latent_output.shape)
        # # added
        # encodings = torch.flatten(fine_grained_latent_output, 2)
        # print("Encodings Before:", encodings.shape)
        # encodings = encodings.permute(0,2,1)
        # print("Encodings After:", encodings.shape)
        
        latent_output = self.avgpool(torch.flatten(x,2))
        # print("Latent Output Before:", latent_output.shape)
        ...
        #     'fine_grained_embedding': encodings,
...
def create_htsat_model(audio_cfg, enable_fusion=False, fusion_type='None'):
    # try:
        ...
        if audio_cfg.model_name == "tiny":
            ...
            print('========================================')
            print("Created a BASE HTSAT model")
            print('========================================')
            model = HTSAT_Swin_Transformer(
                spec_size=256,
                patch_size=4,
