...
class DataCollatorForSeq2Seq:
    ...
    def load_audio(self, filename):
        ...
        sample = self.get_audio_features({}, audio_data, 480000, "fusion", "repeatpad")
    ...
    def __call__(self, features, return_tensors=None):
        ...
        audio_features = {}
        audio_features["mel_fusion"] = torch.stack([item["mel_fusion"] for item in audios], dim=0)
        audio_features["longer"] = torch.stack([item["longer"] for item in audios], dim=0)
        audio_features["waveform"] = torch.stack([item["waveform"] for item in audios], dim=0)
        ...
        new_features["audio_input"] = audio_features

        # adjust the attention mask
        ori_att_mask = new_features["attention_mask"]
        audio_att_mask = torch.ones([ori_att_mask.shape[0], 64]) # batch size [2, 32]
