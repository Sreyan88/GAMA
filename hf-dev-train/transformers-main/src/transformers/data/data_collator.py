...
class DataCollatorForSeq2Seq:
    ...
    def load_audio(self, filename):
        ...
        try:
            ...
            waveform, sr = torch.zeros([1,160000]), 16000
    ...
    def __call__(self, features, return_tensors=None):
        ...
        for key in ['instruction', 'input', 'dataset', 'task', 'output']: #tokenizer_input_bert
            for feature in new_features:
                if key == 'tokenizer_input_bert':
                    bert_features.append(feature[key])
                feature.pop(key)
        ...
        audio_att_mask = torch.ones([ori_att_mask.shape[0], 96]) # batch size [2, 32]
        new_features["attention_mask"] = torch.concat([audio_att_mask, ori_att_mask], dim=1)
        new_features["tokenizer_input_bert"] = bert_features
        return new_features
